	...
	if query := mq.withWhere; query != nil {
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(message.WherePrimaryKey[1], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, mq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "where": %w`, err)
		}
		query.Where(chat.IDIn(edgeids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "where" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Where = append(nodes[i].Edges.Where, n)
			}
		}
	}
