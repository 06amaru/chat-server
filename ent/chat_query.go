...
func (cq *ChatQuery) QueryMembers() *UserQuery {
...
func (cq *ChatQuery) WithMembers(opts ...func(*UserQuery)) *ChatQuery {
	query := (&UserClient{config: cq.config}).Query()
...
func (cq *ChatQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Chat, error) {
	...
	if query := cq.withMembers; query != nil {
		if err := cq.loadMembers(ctx, query, nodes,
			func(n *Chat) { n.Edges.Members = []*User{} },
			func(n *Chat, e *User) { n.Edges.Members = append(n.Edges.Members, e) }); err != nil {
			return nil, err
		}
	}
...
func (cq *ChatQuery) loadMembers(ctx context.Context, query *UserQuery, nodes []*Chat, init func(*Chat), assign func(*Chat, *User)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(chat.MembersTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(chat.MembersPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(chat.MembersPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(chat.MembersPrimaryKey[0]))
...
func (cq *ChatQuery) loadMessages(ctx context.Context, query *MessageQuery, nodes []*Chat, init func(*Chat), assign func(*Chat, *Message)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(chat.MessagesTable)
		s.Join(joinT).On(s.C(message.FieldID), joinT.C(chat.MessagesPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(chat.MessagesPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(chat.MessagesPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
